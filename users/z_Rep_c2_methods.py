...
def EditCompany_Method ( oldemp, newemp, iduser):
    ...
    if sucesso[0] and sucesso[1]:
        ...
        husucess = InsertHistoryEmp ( oldemp, unicode(string) )
        ...
        for item in sucesso[1]:
            ...
            husucess = InsertHistoryEmp    ( oldemp, u'Alteração ' + str(num) +' | Campo: ' + item[0] +" | Para:" + item[2])
...
def NewCompany_Method(dado,iduser):
    ...
    if not exist:
        ...
        if sucesso:
            ...
            hsucess = InsertHistoryUser( iduser , 'Cadastrou a empresa '       + unicode( dado.get('emp_nome') ))
