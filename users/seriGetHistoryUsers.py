...
def HistoryUsuario(iduser,page, filter):
    ...
    if filter == "all":
        ...
        respo = []
        otherpage = 0
        tamanho = len(History.objects.filter(user_nome__icontains=filter, user_ids=iduser))
        res = History.objects.filter(user_nome__icontains=filter, user_ids=iduser)[number1:number]
        if number>tamanho:
            number=tamanho
            otherpage = 1
        respo.append([str(number1+1),str(number),str(tamanho),str(otherpage)])
